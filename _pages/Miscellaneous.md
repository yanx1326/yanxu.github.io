---
layout: page
title: miscellaneous
permalink: /miscellaneous/
nav: true
nav_order: 5
---

Outside research, I enjoy climbing, mountaineering, hiking, skiing, and running.

## Climbing

### Acadia Beehive

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/arcadia.JPG" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

### Bouldering

### Rumney lead

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/rumney_lead.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/rumney_lead_2.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

### Top rope

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/tr_outdoor.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/tr_outdoor_2.JPG" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

## Mountaineering

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/KAN_5134.jpeg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/KAN_5011-2.jpeg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/KAN_4750-1.jpeg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

## Hiking

### Half Dome

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/half_dome.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

## Skiing

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/ski.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/ski_2.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>

## Running

### Cambridge Half Marathon

<div class="row mt-3">
  <div class="col-sm mt-3 mt-md-0">
    {% include figure.liquid path="assets/img/mis/cambridge_half_marathon.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div>
